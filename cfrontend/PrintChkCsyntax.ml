...
let rec name_cdecl id ty =
  ...
  (* TODO: Print checked ptr in the right way *)
  | Tchkcptr(t, a) ->
      let id' =
        match t with
        | Tfunction _ | Tarray _ -> sprintf "(<*>%s%s)" (attributes_space a) id
...
let rec precedence = function
  ...
  | Ebinop((Oeq|ChkCop.One), _, _, _) -> (9, LtoR)
...
let print_typed_value p v ty =
  match v, ty with
  | Vint n, ChkCtypes.Tint(I32, Unsigned, _) ->
  ...
  | Vlong n, ChkCtypes.Tlong(Unsigned, _) ->
...
let print_fundef p id fd =
  match fd with
  | ChkCtypes.External((EF_external _ | EF_runtime _| EF_malloc | EF_free), args, res, cconv) ->
      fprintf p "extern %s;@ @ "
                (name_cdecl (extern_atom id) (Tfunction(args, res, cconv)))
  | ChkCtypes.External(_, _, _, _) ->
      ()
...
let print_fundecl p id fd =
  ...
  | ChkCtypes.Internal f ->
      ...
      fprintf p "%s %s;@ @ " linkage
                (name_cdecl (extern_atom id) (ChkCsyntax.type_of_function f))
  | _ -> ()
...
let print_globvar p id v =
  ...
  | _ ->
      ...
      | (ChkCtypes.Tint _ | ChkCtypes.Tlong _ | ChkCtypes.Tfloat _ | Tpointer _ | Tfunction _),
        [i1] ->
          print_init p i1
      | _, il ->
...
let print_program p prog =
  ...
  List.iter (print_globdecl p) prog.ChkCtypes.prog_defs;
  List.iter (print_globdef p) prog.ChkCtypes.prog_defs;
  fprintf p "@]@."
