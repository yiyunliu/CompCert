...
Require Import ChkCtypes ChkCop ChkCsyntax ChkCsem.
...
Definition type_conditional (ty1 ty2: type) : res type :=
  ...
  (* YL: checked pointer has no special treatment? *)
  | t1, t2 =>
      type_combine t1 t2
  end.
...
Inductive wt_val : val -> type -> Prop :=
  ...
  | wt_val_void: forall v,
      wt_val v Tvoid
  (* YL: TODO This is wrong *)
  | wt_val_chkcpointer: forall b ofs ty a,
      wt_val (Vptr b ofs) (Tchkcptr ty a).
...
Proof.
  intros; red; simpl.
  destruct Archi.ptr64; [ | destruct (ChkCtypes.intsize_eq i2 I32)].
...
Proof with (try discriminate).
  ...
  set (ty' := ChkCop.binarith_type (classify_binarith ty1 ty2)) in *.
