...
(** A deterministic evaluation strategy for Checked C. *)
...
Definition safe (s: ChkCsem.state) : Prop :=
  forall s', star ChkCsem.step ge s E0 s' ->
  (exists r, final_state s' r) \/ (exists t, exists s'', ChkCsem.step ge s' t s'').

Lemma safe_steps:
  forall s s',
  safe s -> star ChkCsem.step ge s E0 s' -> safe s'.
Proof.
  intros; red; intros.
...
Lemma star_safe:
  forall s1 s2 t s3,
  safe s1 -> star ChkCsem.step ge s1 E0 s2 -> (safe s2 -> star ChkCsem.step ge s2 t s3) ->
  star ChkCsem.step ge s1 t s3.
Proof.
...
Lemma plus_safe:
  forall s1 s2 t s3,
  safe s1 -> star ChkCsem.step ge s1 E0 s2 -> (safe s2 -> plus ChkCsem.step ge s2 t s3) ->
  plus ChkCsem.step ge s1 t s3.
Proof.
...
Lemma eval_simple_steps:
   (forall a v, eval_simple_rvalue e m a v ->
    ...
    star ChkCsem.step ge (ExprState f (C a) k e m)
...
Lemma eval_simple_list_steps:
  ...
  star ChkCsem.step ge (ExprState f (C rl) k e m)
...
Lemma estep_simulation:
  forall S t S',
  estep S t S' -> plus ChkCsem.step ge S t S'.
Proof.
  intros. inv H.
(* simple *)
...
(* call *)
  ...
  left; apply ChkCsem.step_call; eauto. econstructor; eauto.
  traceEq. auto.
(* builtin *)
  exploit eval_simple_list_implies; eauto. intros [vl' [A B]].
  eapply plus_right.
  ...
  left; apply ChkCsem.step_rred; eauto. econstructor; eauto.
  traceEq.
Qed.
...
Theorem step_simulation:
  forall S1 t S2,
  step S1 t S2 -> plus ChkCsem.step ge S1 t S2.
Proof.
  intros. inv H.
...
Theorem strategy_simulation:
  forall p, backward_simulation (ChkCsem.semantics p) (semantics p).
Proof.
  intros.
